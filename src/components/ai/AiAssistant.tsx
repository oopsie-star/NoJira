...
export function AiAssistant({ projectName }: AiAssistantProps) {
  ...
  async function runAgentLoopBody(userContent: string, history: ChatMessage[], extraInstruction: string, maxIterations: number, controller: AbortController) {
    ...
    const toolCtx = { members, tasks, epics, aiAgentProfileId, activeProjectId, createEpic, createSprint, createTask, updateTask, downloadAttachmentText }
  ...
  return (
    <>
      ...
      {/* bottom-24 alone drifted under the mobile tab bar on devices with a
          ...
          account for it — tie this to the same nav height the bar itself uses,
          plus a gap, so it can't overlap. */}
      <div className="fixed bottom-[calc(var(--qira-mobile-nav-h)+0.75rem)] right-4 lg:bottom-6 lg:right-6 z-[80]">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
